...
- **Related:** [ADR-phase2-deps-spec-review.md](./ADR-phase2-deps-spec-review.md)
...
existing Vitest. Operators match Stryker's core set:
...
- deps-lock and spec-review remain enforced (see related ADR).
...
- Official `@stryker-mutator/core` + lockfile (follow-up; requires
  `deps-approved`)
