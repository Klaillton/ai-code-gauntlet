...
> **Historical research note (Phase-0/1 snapshot).** Live kit status is in [`PREMISES.md`](./PREMISES.md), the Phase-2 ADRs, and each app’s `AGENTS.md` — mutation, complexity, protect-specs, deps-lock, secrets-scan, and no-cheat are already wired. Do not treat §§9.8–9.11 “falta …” rows as current roadmap.
