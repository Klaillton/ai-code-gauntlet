...
Working-tree `.gauntlet/allow-*` files are **not** grants (agents could create them).
If git is unavailable (`rev-parse` fails), protect-specs **fails** (`git required for this gate`).
...
Working-tree `.gauntlet/allow-deps-edit` is **not** a grant. No git → deps-lock **fails**.
