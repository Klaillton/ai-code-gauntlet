...
hardened (protect-specs, secrets-scan, no-cheat, spec-sync, docs; deps-lock when present).
...
(`protect-specs`, `secrets-scan`, `no-cheat`, `spec-sync`, `docs`, and
`deps-lock` if the template ships it) are always wired; scripts are copied
from the template.
